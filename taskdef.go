...
//
// 推荐在包级集中声明所有任务类型，并把该任务固定的投递参数一并声明为默认选项，
// 调用处便不再重复传递：
//
...
// defaults 为该任务的默认投递选项，会在每次 Enqueue / Register 时自动生效；
// 调用时传入的选项在默认选项之后应用，按「后者覆盖前者」语义可覆盖默认值。
// name 的合法性在 Enqueue、Register 和 Handle 调用时统一校验，
// 因此可以安全地用字面量在包级变量中声明。
func NewTaskType[T any](name string, defaults ...TaskOption) TaskType[T] {
	return TaskType[T]{name: name, defaults: slices.Clone(defaults)}
}

// NewTask 是 NewTaskType 的简写别名，便于日常声明任务类型。
func NewTask[T any](name string, defaults ...TaskOption) TaskType[T] {
	return NewTaskType[T](name, defaults...)
}
...
// 定义时声明的默认选项先生效，opts 在其后应用并可覆盖默认值。
...
// mergeOptions 把定义时的默认选项与调用时选项按序拼接（调用时选项在后，可覆盖默认值）。
// 必须返回全新切片：defaults 可能被多个 goroutine 并发使用，
// 直接 append 会在共享底层数组上产生数据竞争。
func (t TaskType[T]) mergeOptions(opts []TaskOption) []TaskOption {
	if len(t.defaults) == 0 {
		return opts
	}
	...
	return slices.Concat(t.defaults, opts)
}
